...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.GarmentShipping.GarmentShippingInvoice
{
    public class GarmentShippingInvoiceWithHeaderExcelTemplate
    {
        ...
        public MemoryStream GenerateExcelTemplateLFCENTENNIAL(GarmentShippingInvoiceViewModel viewModel, Buyer buyer, BankAccount bank, GarmentPackingListViewModel pl, int timeoffset, List<CostCalculationGarmentViewModel> ccg)
        {
            ...
            var defectiveAllowance = (double)totalAmnts * 0.005;
            var finalAmount = totalAmnts - (decimal)defectiveAllowance;
            ...
            for (var z = 0; z < 4; z++)
            {
                for (var a = 0; a < listMerge.Length; a++)
                {
                    ...
                    //write Defective
                    if (listMerge[a] == 'F' && z == 2)
                    {
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Value = "Defective Allowance 0.5%";
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Style.Font.Bold = true;
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Style.Font.Color.SetColor(System.Drawing.Color.Red);
                    }

                    //write value of Defective
                    if (listMerge[a] == 'K' && z == 2)
                    {
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Value = "US$" + string.Format("{0:n2}", defectiveAllowance);
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Style.Font.Bold = true;
                        sheet.Cells[$"{listMerge[a]}{afterTotalIndex + z}"].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Right;
            ...
            var grandTotalIndex = afterTotalIndex + 4;
            ...
            sheet.Cells[$"K{grandTotalIndex}"].Value = "US$"+ string.Format("{0:n2}", finalAmount);
            ...
            string[] lines1 = new[]
    {
        ...
        "JAWA TENGAH, INDONESIA",
            ...
            if (viewModel.GarmentShippingInvoiceAdjustments.Count > 0)
            {
                ...
                sheet.Cells[$"A{TtltIndex + 2}:H{PaymentIndex + 2}"].Merge = true;
            ...
            else
            {
                if (finalAmount < 0)
                {
                    ...
                    amountToText = "MINUS " + CurrencyToText.ToWords(finalAmount);
                }
                else
                {
                    amountToText = CurrencyToText.ToWords(finalAmount);
                }
