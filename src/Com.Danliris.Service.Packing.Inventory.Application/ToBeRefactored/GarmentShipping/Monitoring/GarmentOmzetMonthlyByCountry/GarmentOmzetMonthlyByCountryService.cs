...
{
    public class GarmentOmzetMonthlyByCountryService : IGarmentOmzetMonthlyByCountryService
    {
        ...
        public GarmentOmzetMonthlyByCountryService(IServiceProvider serviceProvider)
        {
        ...
        public IQueryable<GarmentOmzetMonthlyByCountryViewModel> GetData(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            var newQ = (from a in queryPL
                        ...
                        select new GarmentOmzetMonthlyByCountryListViewModel
                        {
                            ...
                            Amount = c.Amount,
                            Quantity = c.Quantity,
                        }).OrderBy(o => o.Destination).ThenBy(o => o.BuyerName).ThenBy(o => o.TruckingDate).ThenBy(o => o.InvoiceNo);

            var garmentomzets = newQ.ToList()
                .GroupBy(gmtomzt => {
                    var negara = gmtomzt.Destination;
                    return negara;
                })
                .Select(groupomzt => new GarmentOmzetMonthlyByCountryViewModel
                {
                    CountryName = groupomzt.Key,
                    Buyers = groupomzt
                        .GroupBy(ngr => ngr.BuyerName)
                        .Select(groupBuyer => new GarmentOmzetMonthlyByCountryBuyerViewModel
                        {
        ...
        GarmentOmzetMonthlyByCountryDetailViewModel GetGarmentOmzet(GarmentOmzetMonthlyByCountryListViewModel go)
        {
            return new GarmentOmzetMonthlyByCountryDetailViewModel
            {
        ...
        public List<GarmentOmzetMonthlyByCountryViewModel> GetReportData(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            if (data != null && data.Count > 0)
            {
                ...
                foreach (var d in data)
                {
                    ...
                    foreach (var buyer in d.Buyers)
                    {
                        ...
                        {
                            index++;
                            result.Rows.Add(index, d.CountryName, buyer.Buyer, detail.UnitCode, detail.InvoiceNo, detail.TruckingDate.ToString("dd MMMM yyyy", new CultureInfo("id-ID")), detail.RO_Number, detail.ComodityName, detail.ComodityDesc, detail.Quantity, detail.UOMUnit, detail.Amount);
