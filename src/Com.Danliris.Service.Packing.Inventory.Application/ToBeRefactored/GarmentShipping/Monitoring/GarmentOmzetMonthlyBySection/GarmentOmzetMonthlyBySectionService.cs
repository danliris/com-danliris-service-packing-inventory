...
{
    public class GarmentOmzetMonthlyBySectionService : IGarmentOmzetMonthlyBySectionService
    {
        ...
        public GarmentOmzetMonthlyBySectionService(IServiceProvider serviceProvider)
        {
        ...
        public IQueryable<GarmentOmzetMonthlyBySectionViewModel> GetData(string section, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            var newQ = (from a in queryPL
                        ...
                        select new GarmentOmzetMonthlyBySectionListViewModel
                        {
            ...
            var garmentomzets = newQ.ToList()
                .GroupBy(gmtomzt => {
                    var seksi = gmtomzt.SectionCode;
                    return seksi;
                })
                .Select(groupomzt => new GarmentOmzetMonthlyBySectionViewModel
                {
                    SectionCode = groupomzt.Key,
                    Buyers = groupomzt
                        .GroupBy(sie => sie.BuyerName)
                        .Select(groupBuyer => new GarmentOmzetMonthlyBySectionBuyerViewModel
                        {
        ...
        GarmentOmzetMonthlyBySectionDetailViewModel GetGarmentOmzet(GarmentOmzetMonthlyBySectionListViewModel go)
        {
            return new GarmentOmzetMonthlyBySectionDetailViewModel
            {
        ...
        public List<GarmentOmzetMonthlyBySectionViewModel> GetReportData(string section, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            var Query = GetData(section, dateFrom, dateTo, offset);
            return Query.ToList();
        }

        public MemoryStream GenerateExcel(string section, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            var Query = GetData(section, dateFrom, dateTo, offset);
            ...
            result.Columns.Add(new DataColumn() { ColumnName = "S E K S I", DataType = typeof(string) });
            ...
            if (data != null && data.Count > 0)
            {
                ...
                foreach (var d in data)
                {
                    ...
                    foreach (var buyer in d.Buyers)
                    {
                        ...
                        {
                            index++;
                            result.Rows.Add(index, d.SectionCode, buyer.Buyer, detail.UnitCode, detail.InvoiceNo, detail.TruckingDate.ToString("dd MMMM yyyy", new CultureInfo("id-ID")), detail.RO_Number, detail.ComodityName, detail.ComodityDesc, detail.Quantity, detail.UOMUnit, detail.Amount);
