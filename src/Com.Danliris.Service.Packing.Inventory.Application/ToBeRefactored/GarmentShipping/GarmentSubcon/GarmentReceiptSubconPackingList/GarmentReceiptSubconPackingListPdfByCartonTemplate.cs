...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.GarmentShipping.GarmentSubcon.GarmentReceiptSubconPackingList
{
    public class GarmentReceiptSubconPackingListPdfByCartonTemplate
    {
        private IIdentityProvider _identityProvider;

        public GarmentReceiptSubconPackingListPdfByCartonTemplate(IIdentityProvider identityProvider)
        {
            _identityProvider = identityProvider;
        }

        public MemoryStream GeneratePdfTemplate(GarmentReceiptSubconPackingListViewModel viewModel)
        {
            ...
            var newItems = new List<GarmentReceiptSubconPackingListItemViewModel>();
            var newItems2 = new List<GarmentReceiptSubconPackingListItemViewModel>();
            var newDetails = new List<GarmentReceiptSubconPackingListDetailViewModel>();
            ...
            foreach (var d in newDetails)
            {
                if (newItems.Count == 0)
                {
                    ...
                    i.Details = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                    i.Details.Add(d);
                    newItems.Add(i);
                }
                else
                {
                    ...
                    {
                        var y = viewModel.Items.OrderBy(o => o.RONo).Select(a => new GarmentReceiptSubconPackingListItemViewModel
                        {
                            Id = a.Id,
                            RONo = a.RONo,
                            Article = a.Article,
                     ...
                     .Single(a => a.Id == d.PackingListItemId);
                        y.Details = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                        y.Details.Add(d);
                        newItems.Add(y);
                    }
                }
            ...
            var cartons = new List<GarmentReceiptSubconPackingListDetailViewModel>();
            double grandTotal = 0;
            ...
            foreach (var item in newItems2)
            {
                ...
                cellItemContent.Phrase = new Phrase(":", normal_font);
                tableItem.AddCell(cellItemContent);
                cellItemContent.Phrase = new Phrase(viewModel.Buyer.Name, normal_font);
                tableItem.AddCell(cellItemContent);
                cellItemContent.Phrase = new Phrase("", normal_font);
                ...
                var subCartons = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                var subGrossWeight = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                var subNetWeight = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                var subNetNetWeight = new List<GarmentReceiptSubconPackingListDetailViewModel>();
                ...
                foreach (var detail in item.Details)
                {
                    ...
                    {
                        cartons.Add(new GarmentReceiptSubconPackingListDetailViewModel { Carton1 = detail.Carton1, Carton2 = detail.Carton2, CartonQuantity = ctnsQty });
                    }
                    if (subCartons.FindIndex(c => c.Carton1 == detail.Carton1 && c.Carton2 == detail.Carton2 && c.Index == detail.Index) < 0)
                    {
                        subCartons.Add(new GarmentReceiptSubconPackingListDetailViewModel { Carton1 = detail.Carton1, Carton2 = detail.Carton2, CartonQuantity = ctnsQty });
                        subGrossWeight.Add(new GarmentReceiptSubconPackingListDetailViewModel { Carton1 = detail.Carton1, Carton2 = detail.Carton2, CartonQuantity = detail.CartonQuantity, GrossWeight = grossWeight });
                        subNetWeight.Add(new GarmentReceiptSubconPackingListDetailViewModel { Carton1 = detail.Carton1, Carton2 = detail.Carton2, CartonQuantity = detail.CartonQuantity, NetWeight = netWeight });
                        subNetNetWeight.Add(new GarmentReceiptSubconPackingListDetailViewModel { Carton1 = detail.Carton1, Carton2 = detail.Carton2, CartonQuantity = detail.CartonQuantity, NetNetWeight = netNetWeight });
                    }
            ...
            tableGrandTotal.AddCell(new PdfPCell()
            {
                ...
                Phrase = new Phrase($"{totalCtns} {NumberToTextIDN.terbilang(totalCtns).ToUpper()}  DARI {string.Join(" DAN ", comodities)}]", normal_font)
    ...
    class GarmentPackingListDraftPdfByCartonTemplatePageEvent : PdfPageEventHelper
    {
        ...
        private GarmentReceiptSubconPackingListViewModel viewModel;

        public GarmentPackingListDraftPdfByCartonTemplatePageEvent(IIdentityProvider identityProvider, GarmentReceiptSubconPackingListViewModel viewModel)
        {
        ...
        public override void OnStartPage(PdfWriter writer, Document document)
        {
            ...
            cb.ShowTextAligned(PdfContentByte.ALIGN_CENTER, "PACKING LIST TERIMA SUBCON", width / 2, titleY, 0);
