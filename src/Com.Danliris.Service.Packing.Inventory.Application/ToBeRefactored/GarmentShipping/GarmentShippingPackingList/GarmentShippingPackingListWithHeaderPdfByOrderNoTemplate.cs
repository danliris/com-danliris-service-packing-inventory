...
{
    public class GarmentShippingPackingListWithHeaderPdfByOrderNoTemplate
    {
        private IIdentityProvider _identityProvider;

        public GarmentShippingPackingListWithHeaderPdfByOrderNoTemplate(IIdentityProvider identityProvider)
        {
            _identityProvider = identityProvider;
        }

        public MemoryStream GeneratePdfTemplate(GarmentShippingPackingListViewModel viewModel, string fob, string cprice)
        {
            //int maxSizesCount = viewModel.Items.Max(i => i.Details.Max(d => d.Sizes.GroupBy(g => g.Size.Id).Count()));
            var newItems = new List<GarmentShippingPackingListItemViewModel>();
            var newItems2 = new List<GarmentShippingPackingListItemViewModel>();
            var newDetails = new List<GarmentShippingPackingListDetailViewModel>();
            ...
            foreach (var d in newDetails)
            {
                if (newItems.Count == 0)
                {
                    ...
                    i.Details = new List<GarmentShippingPackingListDetailViewModel>();
                    i.Details.Add(d);
                    newItems.Add(i);
                }
                else
                {
                    ...
                    {
                        var y = viewModel.Items.Select(a => new GarmentShippingPackingListItemViewModel
                        {
                            Id = a.Id,
                            RONo = a.RONo,
                            Article = a.Article,
                        ...
                        y.Details = new List<GarmentShippingPackingListDetailViewModel>();
                        y.Details.Add(d);
                        newItems.Add(y);
                    }
                }
            ...
            writer.PageEvent = new GarmentShippingPackingListWithHederPdfByOrderNoPageEvent(_identityProvider, viewModel);
            ...
            foreach (var item in newItems2)
            {
                ...
                tableDetail.AddCell(new PdfPCell()
                {
                    ...
                    Phrase = new Phrase($"      - Sub Ctns = {subCtns}          - Sub G.W. = {String.Format("{0:0.00}", item.Details.Sum(a => a.GrossWeight * a.CartonQuantity))} Kgs          - Sub N.W. = {String.Format("{0:0.00}", item.Details.Sum(a => a.NetWeight * a.CartonQuantity))} Kgs          - Sub N.N.W. = {String.Format("{0:0.00}", item.Details.Sum(a => a.NetNetWeight * a.CartonQuantity))} Kgs          - Sub M.S.R. = {String.Format("{0:0.00}", item.Details.Sum(a => a.Length * a.Width * a.Height * a.CartonQuantity / 1000000))} CBM", normal_font)
                });
    ...
    class GarmentShippingPackingListWithHederPdfByOrderNoPageEvent : PdfPageEventHelper
    {
        private IIdentityProvider identityProvider;
        private GarmentShippingPackingListViewModel viewModel;

        public GarmentShippingPackingListWithHederPdfByOrderNoPageEvent(IIdentityProvider identityProvider, GarmentShippingPackingListViewModel viewModel)
        {
