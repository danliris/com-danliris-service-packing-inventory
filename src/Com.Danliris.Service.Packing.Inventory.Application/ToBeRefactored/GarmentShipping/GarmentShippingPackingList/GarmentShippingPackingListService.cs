...
{
    public class GarmentShippingPackingListService : IGarmentShippingPackingListService
    {
        private const string UserAgent = "GarmentShippingPackingListService";

        protected const string IMG_DIR = "GarmentShippingPackingList";

        protected readonly IGarmentShippingPackingListRepository _packingListRepository;
        ...
        public GarmentShippingPackingListService(IServiceProvider serviceProvider)
        {
            _packingListRepository = serviceProvider.GetService<IGarmentShippingPackingListRepository>();
        ...
        public GarmentShippingPackingListViewModel MapToViewModel(GarmentShippingPackingListModel model)
        {
            var vm = new GarmentShippingPackingListViewModel()
            {
                Active = model.Active,
                Id = model.Id,
                ...
                Items = (model.Items ?? new List<GarmentShippingPackingListItemModel>()).Select(i => new GarmentShippingPackingListItemViewModel
                {
                    Active = i.Active,
                    Id = i.Id,
                    ...
                    Details = (i.Details ?? new List<GarmentShippingPackingListDetailModel>()).Select(d => new GarmentShippingPackingListDetailViewModel
                    {
                        Active = d.Active,
                        Id = d.Id,
                        ...
                        Sizes = (d.Sizes ?? new List<GarmentShippingPackingListDetailSizeModel>()).Select(s => new GarmentShippingPackingListDetailSizeViewModel
                        {
                            Active = s.Active,
                            Id = s.Id,
                ...
                Measurements = (model.Measurements ?? new List<GarmentShippingPackingListMeasurementModel>()).GroupBy(g => new { g.Length, g.Width, g.Height }).Select(m => new GarmentShippingPackingListMeasurementViewModel
                {
                ...
                StatusActivities = (model.StatusActivities ?? new List<GarmentShippingPackingListStatusActivityModel>()).Select(a => new GarmentShippingPackingListStatusActivityViewModel
                {
                    Id = a.Id,
        ...
        protected GarmentShippingPackingListModel MapToModel(GarmentShippingPackingListViewModel viewModel)
        {
            var items = (viewModel.Items ?? new List<GarmentShippingPackingListItemViewModel>()).Select(i =>
            {
                var details = (i.Details ?? new List<GarmentShippingPackingListDetailViewModel>()).Select(d =>
                {
                    var sizes = (d.Sizes ?? new List<GarmentShippingPackingListDetailSizeViewModel>()).Select(s =>
                    {
                        s.Size = s.Size ?? new SizeViewModel();
                        return new GarmentShippingPackingListDetailSizeModel(s.Size.Id, s.Size.Size, s.Size.SizeIdx, s.Quantity) { Id = s.Id };
                    }).ToList();

                    return new GarmentShippingPackingListDetailModel(d.Carton1, d.Carton2, d.Style, d.Colour, d.CartonQuantity, d.QuantityPCS, d.TotalQuantity, d.Length, d.Width, d.Height, d.GrossWeight, d.NetWeight, d.NetNetWeight, sizes, d.Index) { Id = d.Id };
                }).ToList();
                ...
                return new GarmentShippingPackingListItemModel(i.ExpenditureGoodNo, i.RONo, i.SCNo, i.BuyerBrand.Id, i.BuyerBrand.Name, i.Comodity.Id, i.Comodity.Code, i.Comodity.Name, i.ComodityDescription, i.MarketingName, i.Quantity, i.Uom.Id.GetValueOrDefault(), i.Uom.Unit, i.PriceRO, i.Price, i.PriceFOB, i.PriceCMT, i.Amount, i.Valas, i.Unit.Id, i.Unit.Code, i.Article, i.OrderNo, i.Description, i.DescriptionMd, i.Remarks, i.RoType, details)
                {
                    Id = i.Id
                };
            }).ToList();

            var measurements = (viewModel.Measurements ?? new List<GarmentShippingPackingListMeasurementViewModel>()).Select(m => new GarmentShippingPackingListMeasurementModel(m.Length, m.Width, m.Height, m.CartonsQuantity, m.CreatedBy)
            {
                Id = m.Id
            }).ToList();
        ...
        public virtual async Task<string> Create(GarmentShippingPackingListViewModel viewModel)
        {
            viewModel.Status = GarmentShippingPackingListStatusEnum.CREATED.ToString();

            GarmentShippingPackingListModel garmentPackingListModel = MapToModel(viewModel);
            ...
            garmentPackingListModel.StatusActivities.Add(new GarmentShippingPackingListStatusActivityModel(_identityProvider.Username, UserAgent, garmentPackingListModel.Status.ToString()));
        ...
        private string GenerateInvoiceNo(GarmentShippingPackingListViewModel viewModel)
        {
        ...
        public virtual ListResult<GarmentShippingPackingListViewModel> Read(int page, int size, string filter, string order, string keyword)
        {
            var query = _packingListRepository.ReadAll();

            Dictionary<string, object> FilterDictionary = JsonConvert.DeserializeObject<Dictionary<string, object>>(filter);
            query = QueryHelper<GarmentShippingPackingListModel>.Filter(query, FilterDictionary);
            ...
            query = QueryHelper<GarmentShippingPackingListModel>.Order(query, OrderDictionary);
            ...
            return new ListResult<GarmentShippingPackingListViewModel>(data, page, size, query.Count());
        }

        public virtual async Task<GarmentShippingPackingListViewModel> ReadById(int id)
        {
        ...
        public async Task<GarmentShippingPackingListViewModel> ReadByInvoiceNo(string no)
        {
        ...
        public ListResult<GarmentShippingPackingListViewModel> ReadNotUsed(int page, int size, string filter, string order, string keyword)
        {
            var query = _packingListRepository.ReadAll();
            List<string> SearchAttributes = new List<string>()
            {
            ...
            query = QueryHelper<GarmentShippingPackingListModel>.Search(query, SearchAttributes, keyword);
        ...
        public ReadResponse<dynamic> ReadSampleDelivered(int page, int size, string filter, string order, string keyword, string Select = "{}")
        {
            ...
            IQueryable  SelectedQuery = QueryHelper<GarmentShippingPackingListModel>.ConfigureSelect(query, SelectDictionary);
        ...
        public ListResult<GarmentShippingPackingListViewModel> ReadNotUsedCostStructure(int page, int size, string filter, string order, string keyword)
        {
            var query = _packingListRepository.ReadAll();
            List<string> SearchAttributes = new List<string>()
            {
		...
		public virtual async Task<int> Update(int id, GarmentShippingPackingListViewModel viewModel)
		{
		...
		public virtual async Task<int> Unpost(int id, GarmentShippingPackingListViewModel viewModel)
		{
			GarmentShippingPackingListModel garmentPackingListModel = MapToModel(viewModel);
  ...
  //          var data = await _packingListRepository.ReadByIdAsync(id);
  //          var offset = 7;
  //          var PdfTemplate = new GarmentShippingPackingListDraftPdfByCartonMDTemplate(_identityProvider);
  ...
  //      }
        ...
        public virtual async Task<MemoryStreamResult> ReadPdfById(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListPdfTemplate(_identityProvider);
        ...
        public virtual async Task<MemoryStreamResult> ReadWHPdfById(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListWithHeadePdfTemplate(_identityProvider);
        ...
        public virtual async Task<MemoryStreamResult> ReadWHSectionDPdfById(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListWithHeadeSectionDPdfTemplate(_identityProvider);
            var fob = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice == "FOB" || s.CPrice == "FCA" ? s.From : s.To).FirstOrDefault();
            var cPrice = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice).FirstOrDefault();
        ...
        //    var PdfTemplate = new GarmentShippingPackingListDraftPdfByCartonTemplate(_identityProvider);
        ...
        public virtual async Task<MemoryStreamResult> ReadPdfByOrderNo(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListPdfByOrderNoTemplate(_identityProvider);
            var fob = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice == "FOB" || s.CPrice == "FCA" ? s.From : s.To).FirstOrDefault();
            var cPrice = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice).FirstOrDefault();
        ...
        public virtual async Task<MemoryStreamResult> ReadWHPdfByOrderNo(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListWithHeaderPdfByOrderNoTemplate(_identityProvider);
            var fob = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice == "FOB" || s.CPrice == "FCA" ? s.From : s.To).FirstOrDefault();
            var cPrice = _invoiceRepository.ReadAll().Where(w => w.PackingListId == data.Id).Select(s => s.CPrice).FirstOrDefault();
        ...
        public virtual async Task<MemoryStreamResult> ReadWHSectionDPdfByOrderNo(int id)
        {
            ...
            var PdfTemplate = new GarmentShippingPackingListWithHeaderPdfByOrderNoSectionDTemplate(_identityProvider);
        ...
        public virtual async Task<MemoryStreamResult> ReadExcelById(int id)
        {
            ...
            var ExcelTemplate = new GarmentShippingPackingListExcelTemplate(_identityProvider);
        ...
        //    var ExcelTemplate = new GarmentShippingPackingListDraftExcelByCartonTemplate(_identityProvider);
        ...
        public async Task SetPost(List<int> ids)
        {
            ...
            foreach (var model in models)
            {
                ...
                model.SetStatus(GarmentShippingPackingListStatusEnum.POSTED, _identityProvider.Username, UserAgent);
                model.StatusActivities.Add(new GarmentShippingPackingListStatusActivityModel(_identityProvider.Username, UserAgent, GarmentPackingListStatusEnum.POSTED.ToString()));
        ...
        public async Task SetUnpost(int id)
        {
            ...
            model.SetStatus(GarmentShippingPackingListStatusEnum.CREATED, _identityProvider.Username, UserAgent);
            model.StatusActivities.Add(new GarmentShippingPackingListStatusActivityModel(_identityProvider.Username, UserAgent, GarmentPackingListStatusEnum.CREATED.ToString()));
		...
		public async Task SetApproveMd(int id, GarmentShippingPackingListViewModel viewModel)
        {
            GarmentShippingPackingListModel garmentPackingListModel = MapToModel(viewModel);

            await _packingListRepository.UpdateAsync(id, garmentPackingListModel);

            var status = GarmentShippingPackingListStatusEnum.APPROVED_MD;
            ...
            if (oldModel.Status != status)
            {
                ...
                oldModel.StatusActivities.Add(new GarmentShippingPackingListStatusActivityModel(_identityProvider.Username, UserAgent, status.ToString()));
        ...
        public async Task SetApproveShipping(int id, GarmentShippingPackingListViewModel viewModel)
        {
            GarmentShippingPackingListModel garmentPackingListModel = MapToModel(viewModel);

            await _packingListRepository.UpdateAsync(id, garmentPackingListModel);

            var status = GarmentShippingPackingListStatusEnum.APPROVED_SHIPPING;
            var oldModel = _packingListRepository.Query.Single(m => m.Id == id);
            if (oldModel.Status != status)
            {
                oldModel.SetStatus(status, _identityProvider.Username, UserAgent);
        ...
        public Task SetStatus(int id, GarmentShippingPackingListStatusEnum status, string remark = null)
        {
            var model = _packingListRepository.Query.Single(m => m.Id == id);
            model.SetStatus(status, _identityProvider.Username, UserAgent);
            model.StatusActivities.Add(new GarmentShippingPackingListStatusActivityModel(_identityProvider.Username, UserAgent, status.ToString(), remark));
