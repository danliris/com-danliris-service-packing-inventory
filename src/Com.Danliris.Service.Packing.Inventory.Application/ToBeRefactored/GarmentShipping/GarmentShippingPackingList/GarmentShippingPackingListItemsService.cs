...
{
    public class GarmentShippingPackingListItemsService : GarmentShippingPackingListService, IGarmentShippingPackingListItemsService
    {
        private const string UserAgent = "GarmentShippingPackingListItemsService";

        public GarmentShippingPackingListItemsService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        public override ListResult<GarmentShippingPackingListViewModel> Read(int page, int size, string filter, string order, string keyword)
        {
            var query = _packingListRepository.ReadAll();

            GarmentShippingPackingListStatusEnum[] statusFilter = {
                GarmentShippingPackingListStatusEnum.DRAFT,
                GarmentShippingPackingListStatusEnum.DRAFT_POSTED,
                GarmentShippingPackingListStatusEnum.DRAFT_REJECTED_MD,
                GarmentShippingPackingListStatusEnum.DRAFT_APPROVED_MD,
                GarmentShippingPackingListStatusEnum.DRAFT_REJECTED_SHIPPING,
                GarmentShippingPackingListStatusEnum.DRAFT_APPROVED_SHIPPING,
            };
        ...
        public override async Task<GarmentShippingPackingListViewModel> ReadById(int id)
        {
            var viewModel = await _packingListRepository.Query.Select(model => new GarmentShippingPackingListViewModel
            {
                Active = model.Active,
                Id = model.Id,
                ...
                Items = model.Items.Where(i => i.CreatedBy == _identityProvider.Username).Select(i => new GarmentShippingPackingListItemViewModel
                {
                    Active = i.Active,
                    Id = i.Id,
                    ...
                    Remarks = i.Remarks,
                    RoType = i.RoType,
                    Details = i.Details.Select(d => new GarmentShippingPackingListDetailViewModel
                    {
                        Active = d.Active,
                        Id = d.Id,
                        ...
                        Sizes = d.Sizes.Select(s => new GarmentShippingPackingListDetailSizeViewModel
                        {
                            Active = s.Active,
                            Id = s.Id,
                ...
                Measurements = model.Measurements.Where(i => i.CreatedBy == _identityProvider.Username).Select(m => new GarmentShippingPackingListMeasurementViewModel
                {
                    Active = m.Active,
                    Id = m.Id,
                ...
                StatusActivities = model.StatusActivities.Select(a => new GarmentShippingPackingListStatusActivityViewModel
                {
                    Id = a.Id,
        ...
        public override Task<int> Update(int id, GarmentShippingPackingListViewModel viewModel)
        {
            GarmentShippingPackingListModel model = MapToModel(viewModel);
            ...
            var measurements = modelToUpdate.Items
                ...
                .GroupBy(m => new { m.Length, m.Width, m.Height, m.CreatedBy }, (k, g) => new GarmentShippingPackingListMeasurementModel(k.Length, k.Width, k.Height, g.Distinct().Sum(d => d.CartonQuantity), k.CreatedBy));
            ...
            foreach (var itemToUpdate in modelToUpdate.Items.Where(i => i.CreatedBy == _identityProvider.Username))
            {
                ...
                if (item != null)
                {
                    itemToUpdate.SetExpenditureGoodNo(item.ExpenditureGoodNo, _identityProvider.Username, UserAgent);
