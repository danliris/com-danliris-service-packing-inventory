...
{
    public class GarmentShippingExportSalesNoteViewModel : BaseViewModel, IValidatableObject
    {
        ...
        public ICollection<GarmentShippingExportSalesNoteItemViewModel> items { get; set; }

        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if(exportSalesContractId==0 || string.IsNullOrWhiteSpace(salesContractNo))
            {
                yield return new ValidationResult("Sales Contract Export tidak boleh kosong", new List<string> { "salesContract" });
            }
