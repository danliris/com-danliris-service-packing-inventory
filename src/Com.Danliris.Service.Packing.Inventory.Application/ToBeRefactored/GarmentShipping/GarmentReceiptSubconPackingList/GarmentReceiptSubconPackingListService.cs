...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.GarmentShipping.GarmentReceiptSubconPackingList
{
    public class GarmentReceiptSubconPackingListService : IGarmentReceiptSubconPackingListService
    {
        ...
        public GarmentReceiptSubconPackingListService(IServiceProvider serviceProvider)
        {
        ...
        public virtual async Task<string> Create(GarmentReceiptSubconPackingListViewModel viewModel)
        {
            ...
            List<string> packingOutItemIds = new List<string>();
            foreach (var item in garmentPackingListModel.Items)
            {
                ...
                foreach (var detail in item.Details)
                {
                    ...
                    {
                        packingOutItemIds.Add(size.PackingOutItemId.ToString());
            ...
            if(packingOutItemIds.Count() > 0)
            {
                await PutIsPackingListGarmentPackingOut(packingOutItemIds,true);
            }
        ...
        public async Task<int> Delete(int id)
        {
            ...
            if(packingOutItemIds.Count() > 0)
            {
                await PutIsPackingListGarmentPackingOut(packingOutItemIds, false);
            }
        ...
        public async Task<int> Update(int id, GarmentReceiptSubconPackingListViewModel viewModel)
        {
            ...
            List<string> SetTruePackingOutItemIds = new List<string>();
            List<string> SetFalsePackingOutItemIds = new List<string>();
            foreach (var itemToUpdate in modelToUpdate.Items)
            {
                ...
                if(item != null)
                {
                    //itemToUpdate.SetPackingOutNo(item.PackingOutNo, _identityProvider.Username, UserAgent);
                    ...
                    foreach (var detailToUpdate in itemToUpdate.Details)
                    {
                        ...
                        if (detail != null)
                        {
                            ...
                            foreach (var sizeToUpdate in detailToUpdate.Sizes)
                            {
                                ...
                                if (size != null)
                                {
                                    ...
                                    //sizeToUpdate.SetPackingOutItemId(size.PackingOutItemId, _identityProvider.Username, UserAgent);
                                    
                                    //SetFalsePackingOutItemIds.Add(sizeToUpdate.PackingOutItemId.ToString());
                                    //SetTruePackingOutItemIds.Add(size.PackingOutItemId.ToString());
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            {
                                SetFalsePackingOutItemIds.Add(size.PackingOutItemId.ToString());
            ...
            if (SetTruePackingOutItemIds.Count() > 0)
            {
                var distictIds = SetTruePackingOutItemIds.Distinct().ToList();
                await PutIsPackingListGarmentPackingOut(distictIds, true);
            }

            if (SetFalsePackingOutItemIds.Count() > 0)
            {
                var distictIds = SetFalsePackingOutItemIds.Distinct().ToList();
                await PutIsPackingListGarmentPackingOut(distictIds, false);
            }
        ...
        protected async Task<string> PutIsPackingListGarmentPackingOut(List<string> id,bool isPackingList)
        {
            var garmentUnitExpenditureNoteUri = ApplicationSetting.ProductionEndpoint + $"subcon-packing-outs/isPackingList";
            var garmentUnitExpenditureNoteResponse = await _http.PutAsync(garmentUnitExpenditureNoteUri, _identityProvider.Token, new StringContent(JsonConvert.SerializeObject(new { ids = id, isPackingList = isPackingList }), Encoding.UTF8, "application/json"));
