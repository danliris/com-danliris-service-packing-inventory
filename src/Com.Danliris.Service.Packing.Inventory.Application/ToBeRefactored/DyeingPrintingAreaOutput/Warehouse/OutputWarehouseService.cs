    ...
    public class OutputWarehouseService : IOutputWarehouseService
    {
        ...
        private async Task<int> CreateOut(OutputWarehouseViewModel viewModel)
        {
            ...
            if (model == null)
            {
                ...
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, viewModel.Area, OUT, model.Id, model.BonNo, item.ProductionOrder.Id, item.ProductionOrderNo,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.Balance, item.Id, item.ProductionOrder.Type, item.Grade);
        ...
        private async Task<int> CreateAdj(OutputWarehouseViewModel viewModel)
        {
            ...
            var model = new DyeingPrintingAreaOutputModel(viewModel.Date, viewModel.Area, viewModel.Shift, bonNo, true, "", viewModel.Group, type,
                        ...
                        s.MaterialWidth, s.AdjDocumentNo)).ToList());
