    ...
    public class OutputWarehouseService : IOutputWarehouseService
    {
        ...
        public List<InputSppWarehouseViewModel> GetInputSppWarehouseItemListV2(long productionOrderId)
        {
            ...
            if (productionOrderId == 0)
            {
                ...
                query = _inputProductionOrderRepository.ReadAll().OrderByDescending(s => s.LastModifiedUtc).Where(s => s.Area == DyeingPrintingArea.GUDANGJADI && !s.HasOutputDocument && s.PackagingQty != 0 && s.IsFromStockOpname).Take(100);
            }
            else
            {
                query = _inputProductionOrderRepository.ReadAll().OrderByDescending(s => s.LastModifiedUtc).Where(s => s.Area == DyeingPrintingArea.GUDANGJADI && !s.HasOutputDocument && s.ProductionOrderId == productionOrderId && s.PackagingQty != 0 && s.IsFromStockOpname).Take(100);
