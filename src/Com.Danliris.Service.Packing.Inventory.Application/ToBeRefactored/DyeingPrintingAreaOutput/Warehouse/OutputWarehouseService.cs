...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.DyeingPrintingAreaOutput.Warehouse
{
    public class OutputWarehouseService : IOutputWarehouseService
    {
            ...
            if (model.Type == null || model.Type == OUT)
            {
                vm = new OutputWarehouseViewModel()
                {
                    ...
                    WarehousesProductionOrders = model.DyeingPrintingAreaOutputProductionOrders.Select(s => new OutputWarehouseProductionOrderViewModel()
                    {
                        ...
                        Quantity = s.PackagingQty == 0 ? 0 : Convert.ToDecimal(s.Balance) / s.PackagingQty,
        ...
        private string GenerateBonNoAdj(int totalPreviousData, DateTimeOffset date, IEnumerable<double> qtys)
        {
            if (qtys.All(s => s > 0))
            {
                return string.Format("{0}.{1}.{2}.{3}", ADJ_IN, GJ, date.ToString("yy"), totalPreviousData.ToString().PadLeft(4, '0'));
            }
            else
            {
                return string.Format("{0}.{1}.{2}.{3}", ADJ_OUT, GJ, date.ToString("yy"), totalPreviousData.ToString().PadLeft(4, '0'));
            }
        }
                ...
                model = new DyeingPrintingAreaOutputModel(viewModel.Date,
                                                          ...
                                                          viewModel.WarehousesProductionOrders.Select(s =>
                                                            new DyeingPrintingAreaOutputProductionOrderModel(s.ProductionOrder.Id,
                                                                                                             ...
                                                                                                             s.MaterialWidth,
                                                                                                             s.AdjDocumentNo,
                                                                                                              s.ProcessType.Id, s.ProcessType.Name, s.YarnMaterial.Id, s.YarnMaterial.Name, s.ProductSKUId, s.FabricSKUId, s.ProductSKUCode, s.HasPrintingProductSKU, s.ProductPackingId, s.FabricPackingId, s.ProductPackingCode, s.HasPrintingProductPacking)).ToList());
            ...
            else
            {
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    var modelItem = new DyeingPrintingAreaOutputProductionOrderModel(item.ProductionOrder.Id,
                                                                                        ...
                                                                                        item.MaterialWidth,
                                                                                        item.AdjDocumentNo,
                                                                                         item.ProcessType.Id, item.ProcessType.Name, item.YarnMaterial.Id, item.YarnMaterial.Name, item.ProductSKUId, item.FabricSKUId, item.ProductSKUCode, item.HasPrintingProductSKU, item.ProductPackingId, item.FabricPackingId, item.ProductPackingCode, item.HasPrintingProductPacking);
        ...
        private async Task<int> CreateAdj(OutputWarehouseViewModel viewModel)
        {
            ...
            {
                int totalCurrentYearData = _outputRepository.ReadAllIgnoreQueryFilter().Count(s => s.Area == GUDANGJADI && s.Type == ADJ_IN && s.CreatedUtc.Year == viewModel.Date.Year);
                bonNo = GenerateBonNoAdj(totalCurrentYearData + 1, viewModel.Date, viewModel.WarehousesProductionOrders.Select(d => d.Balance));
                type = ADJ_IN;
            }
            else
            {
                int totalCurrentYearData = _outputRepository.ReadAllIgnoreQueryFilter().Count(s => s.Area == GUDANGJADI && s.Type == ADJ_OUT && s.CreatedUtc.Year == viewModel.Date.Year);
                bonNo = GenerateBonNoAdj(totalCurrentYearData + 1, viewModel.Date, viewModel.WarehousesProductionOrders.Select(d => d.Balance));
                type = ADJ_OUT;
            }

            DyeingPrintingAreaOutputModel model = _outputRepository.GetDbSet().AsNoTracking()
                    .FirstOrDefault(s => s.Area == GUDANGJADI && s.Date.Date == viewModel.Date.Date & s.Shift == viewModel.Shift && s.Type == type);
            ...
            else
            {
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    var modelItem = new DyeingPrintingAreaOutputProductionOrderModel(item.ProductionOrder.Id, item.ProductionOrder.No,
                        ...
                        item.HasPrintingProductSKU, item.ProductPackingId, item.FabricPackingId, item.ProductPackingCode, item.HasPrintingProductPacking);
                    modelItem.DyeingPrintingAreaOutputId = model.Id;
        ...
        private MemoryStream GenerateExcelAdj(DyeingPrintingAreaOutputModel model)
        {
            ...
            else
            {
                foreach (var item in query)
                {
                    var qty = item.PackagingQty == 0 ? 0 : Convert.ToDecimal(item.Balance) / item.PackagingQty;
                    dt.Rows.Add(item.ProductionOrderNo, item.ProductionOrderOrderQuantity.ToString("N2", CultureInfo.InvariantCulture), item.ProductionOrderType,
                        ...
                        item.PackagingUnit, item.UomUnit, qty.ToString("N2", CultureInfo.InvariantCulture), item.Balance.ToString("N2", CultureInfo.InvariantCulture), item.AdjDocumentNo, "");
        ...
        public ListResult<AdjWarehouseProductionOrderViewModel> GetDistinctAllProductionOrder(int page, int size, string filter, string order, string keyword)
        {
            ...
            var data = query.ToList()
                ...
                .Select(s => new AdjWarehouseProductionOrderViewModel()
                {
                    ...
                    PackagingQty = s.PackagingQty,
                    PackagingType = s.PackagingType,
                    Quantity = s.PackagingQty == 0 ? 0 : s.Balance / Convert.ToDouble(s.PackagingQty),
                    ProductSKUId = s.ProductSKUId,
                    FabricSKUId = s.FabricSKUId,
                    ProductSKUCode = s.ProductSKUCode,
                    HasPrintingProductSKU = s.HasPrintingProductSKU,
        ...
        private async Task<int> UpdateOut(int id, OutputWarehouseViewModel viewModel)
        {
            ...
            var model = new DyeingPrintingAreaOutputModel(viewModel.Date, viewModel.Area, viewModel.Shift, viewModel.BonNo, viewModel.HasNextAreaDocument, viewModel.DestinationArea,
                    ...
                    s.ProductPackingCode, s.HasPrintingProductPacking)
                    {
                        Id = s.Id
                    }).ToList());
        ...
        public List<InputSppWarehouseViewModel> GetInputSppWarehouseItemListV2(long productionOrderId)
        {
            ...
            if(productionOrderId == 0)
            {
                ...
                query = _inputProductionOrderRepository.ReadAll().OrderByDescending(s => s.LastModifiedUtc).Where(s => s.Area == GUDANGJADI && !s.HasOutputDocument);
            }
            else
            {
                query = _inputProductionOrderRepository.ReadAll().OrderByDescending(s => s.LastModifiedUtc).Where(s => s.Area == GUDANGJADI && !s.HasOutputDocument && s.ProductionOrderId == productionOrderId);
