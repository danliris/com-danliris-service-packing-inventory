...
{
    public class RegradingResultDocReportService : IRegradingResultDocReportService
    {
        ...
        public RegradingResultDocReportService(IServiceProvider serviceProvider)
        {
        ...
        public List<RegradingResultDocReportViewModel> GetReportData(string orderNo, DateTime startdate, DateTime finishdate, int offset)
        {
            var Query = GetReportQuery(orderNo, startdate, finishdate, offset);
            return Query.ToList();
        }

        private List<RegradingResultDocReportViewModel> GetReportQuery(string orderNo, DateTime startdate, DateTime finishdate, int offset)
        {
            ...
            var joinQuery = from a in query
                            ...
                            && b.DestinationArea.ToUpper()=="PACKING" && (b.CartNo.ToUpper().Contains("GRADE") || b.CartNo.ToUpper().Contains("REG"))
                            select new RegradingResultDocReportViewModel
                            {
                                bonNo = a.BonNo,
