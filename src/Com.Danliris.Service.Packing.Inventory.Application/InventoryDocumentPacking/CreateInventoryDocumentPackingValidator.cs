...
{
    public class CreateInventoryDocumentPackingValidator : AbstractValidator<CreateInventoryDocumentPackingViewModel>
    {
        public CreateInventoryDocumentPackingValidator()
        {
            ...
            RuleForEach(inventoryDocument => inventoryDocument.Items).ChildRules(items =>
            {
                ...
                items.RuleFor(item => item.PackingId.GetValueOrDefault()).GreaterThan(0).WithMessage("Pack harus diisi!");
