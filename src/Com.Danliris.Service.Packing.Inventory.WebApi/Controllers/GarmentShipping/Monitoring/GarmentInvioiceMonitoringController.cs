...
namespace Com.Danliris.Service.Packing.Inventory.WebApi.Controllers.GarmentShipping.Monitoring
{
    ...
    public class GarmenInvoiceMonitoringController : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult GetReport(string buyerAgent, string invoiceType, DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order = "{}")
        {
            int offset = Convert.ToInt32(Request.Headers["x-timezone-offset"]);
            string accept = Request.Headers["Accept"];
            try
            {
                var data = _service.GetReportData(buyerAgent, invoiceType, dateFrom, dateTo, offset);
        ...
        [HttpGet("download")]
        public IActionResult GetXls(string buyerAgent, string invoiceType, DateTime? dateFrom, DateTime? dateTo)
        {
            try
            {
                byte[] xlsInBytes;
                ...
                var xls = _service.GenerateExcel(buyerAgent, invoiceType, dateFrom, dateTo, offset);
