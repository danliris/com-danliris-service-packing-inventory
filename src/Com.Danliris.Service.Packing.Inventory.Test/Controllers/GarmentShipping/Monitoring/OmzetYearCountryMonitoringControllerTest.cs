...
{
    public class OmzetYearCountryMonitoringControllerTest
    {
        protected OmzetYearCountryController GetController(IOmzetYearCountryService service, IIdentityProvider identityProvider)
        {
            ...
            var controller = new OmzetYearCountryController(service, identityProvider)
            {
        ...
        {
            var serviceMock = new Mock<IOmzetYearCountryService>();
            serviceMock
                .Setup(s => s.GetReportData(It.IsAny<int>()))
                .Returns(new OmzetYearCountryViewModel());
