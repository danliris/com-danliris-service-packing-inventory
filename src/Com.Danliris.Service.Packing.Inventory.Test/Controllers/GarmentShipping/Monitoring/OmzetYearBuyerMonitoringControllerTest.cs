...
{
    public class OmzetYearBuyerMonitoringControllerTest
    {
        protected OmzetYearBuyerController GetController(IOmzetYearBuyerService service, IIdentityProvider identityProvider)
        {
            ...
            var controller = new OmzetYearBuyerController(service, identityProvider)
            {
        ...
        {
            var serviceMock = new Mock<IOmzetYearBuyerService>();
            serviceMock
                .Setup(s => s.GetReportData(It.IsAny<int>()))
                .Returns(new OmzetYearBuyerViewModel());
