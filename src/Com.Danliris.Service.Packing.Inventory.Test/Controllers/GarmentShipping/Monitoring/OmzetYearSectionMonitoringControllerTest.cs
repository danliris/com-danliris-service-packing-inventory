...
{
    public class OmzetYearSectionMonitoringControllerTest
    {
        protected OmzetYearSectionController GetController(IOmzetYearSectionService service, IIdentityProvider identityProvider)
        {
            var claimPrincipal = new Mock<ClaimsPrincipal>();
            var claims = new Claim[]
            {
                new Claim("username", "Sectiontestusername")
            };
            claimPrincipal.Setup(claim => claim.Claims).Returns(claims);

            var controller = new OmzetYearSectionController(service, identityProvider)
            {
            ...
            controller.ControllerContext.HttpContext.Request.Headers["Authorization"] = "Bearer Sectiontesttoken";
            controller.ControllerContext.HttpContext.Request.Headers["x-timezone-offset"] = $"{It.IsAny<int>()}";
            controller.ControllerContext.HttpContext.Request.Path = new PathString("/v1/Section-test");
        ...
        {
            var serviceMock = new Mock<IOmzetYearSectionService>();
            serviceMock
                .Setup(s => s.GetReportData(It.IsAny<int>()))
                .Returns(new OmzetYearSectionViewModel());
