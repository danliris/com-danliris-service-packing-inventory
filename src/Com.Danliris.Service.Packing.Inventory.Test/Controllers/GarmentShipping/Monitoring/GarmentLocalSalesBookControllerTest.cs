...
{
    public class GarmentLocalSalesBookControllerTest
    {
        protected GarmenLocalSalesBookController GetController(IGarmentLocalSalesBookService service, IIdentityProvider identityProvider)
        {
            ...
            var controller = new GarmenLocalSalesBookController(service, identityProvider)
            {
        ...
        private GarmentLocalSalesBookViewModel ViewModel
        {
            get
            {
                return new GarmentLocalSalesBookViewModel
                {
                    LSNo = "",
                    LSDate = new DateTime(1970, 1, 1),
                    BuyerCode = "",
                    BuyerName = "",
                    TransactionCode = "",
                    TransactionType = "",
        ...
        {
            var serviceMock = new Mock<IGarmentLocalSalesBookService>();
            _ = serviceMock
                .Setup(s => s.GetReportData(It.IsAny<DateTime>(), It.IsAny<DateTime>(), It.IsAny<int>()))
                .Returns(Tuple.Create(new List<GarmentLocalSalesBookViewModel> { ViewModel }, 25));
