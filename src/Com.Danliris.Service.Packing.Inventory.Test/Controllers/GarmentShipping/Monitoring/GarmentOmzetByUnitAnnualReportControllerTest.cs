...
{
    public class GarmentOmzetByUnitAnnualReportControllerTest
    {
        protected GarmentOmzetAnnualByUnitReportController GetController(IGarmentOmzetAnnualByUnitReportService service, IIdentityProvider identityProvider)
        {
            ...
            var controller = new GarmentOmzetAnnualByUnitReportController(service, identityProvider)
            {
        ...
        {
            var serviceMock = new Mock<IGarmentOmzetAnnualByUnitReportService>();
            serviceMock
                .Setup(s => s.GetReportData(It.IsAny<int>(), It.IsAny<int>()))
                .Returns(new ListResult<AnnualOmzetByUnitViewModel>(new List<AnnualOmzetByUnitViewModel>() { new AnnualOmzetByUnitViewModel() }, 1, 1, 1));
            ...
            var response = controller.GetReport(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<int>(), "{}");
        ...
        public void GetGenerateExcel_Success()
        {
            ...
            var response = controller.GetXls(DateTimeOffset.Now.Date.Year);
        ...
        public void GetGenerateExcel_Error()
        {
            ...
            var controller = GetController(service, identityProvider);
            controller.ControllerContext.HttpContext.Request.Headers["Accept"] = "application/xls";
            var response = controller.GetXls(0);
