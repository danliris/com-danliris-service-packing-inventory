...
{
    public class GarmentShippingPackingListControllerSetRevisedMdTest : GarmentShippingPackingListControllerTest
    {
        [Fact]
		...
		[Fact]
		public async Task Set_RevisedToMd_Success()
		{
			var serviceMock = new Mock<IGarmentShippingPackingListService>();
			serviceMock
				.Setup(s => s.SetStatus(It.IsAny<int>(), It.IsAny<GarmentShippingPackingListStatusEnum>(), It.IsAny<string>()))
			...
			var response = await controller.SetRevisedToMd(It.IsAny<int>(), "Alasan");

			Assert.Equal((int)HttpStatusCode.OK, GetStatusCode(response));
		}

		[Fact]
		public async Task Set_RevisedToMd_Exception_BadRequest()
		{
			var serviceMock = new Mock<IGarmentShippingPackingListService>();
			serviceMock
				.Setup(s => s.SetStatus(It.IsAny<int>(), It.IsAny<GarmentShippingPackingListStatusEnum>(), It.IsAny<string>()))
			...
			var response = await controller.SetRevisedToMd(It.IsAny<int>(), It.IsAny<string>());

			Assert.Equal((int)HttpStatusCode.BadRequest, GetStatusCode(response));
		}

		[Fact]
		public async Task Set_RevisedToMd_Exception_InternalServerError()
		{
			var serviceMock = new Mock<IGarmentShippingPackingListService>();
			serviceMock
				.Setup(s => s.SetStatus(It.IsAny<int>(), It.IsAny<GarmentShippingPackingListStatusEnum>(), It.IsAny<string>()))
